    ...
    await axios.get(BASE_URL + "/view/get/" + urlId + "?mode=" + mode)
      .then(response => {
        setViewsChart(
          ...
          sortWeekdaysOrMonths(response.data.param.views_chart.value) : 
          response.data.param.views_chart.value
        )
        setCountryChart({
  ...
  function sortWeekdaysOrMonths(jsonData: any) {
    const sortedData = Object.entries(jsonData)
      .sort(([keyA, valueA], [keyB, valueB]) => {
        const order = isWeekDay(jsonData) ? 
        ...
        isLoading || occuredError ? (
            ...
            <div className='mt-24 items-center justify-around flex w-screen' >
              <div>
                ...
                <h2 
                  ...
                  className="text-center text-[#404727] font-extrabold font-noto">An Error occured while loading your dashboard :(. 
                  <span 
          ...
          <div>
            <div className='pl-20 p-10 none-flex items-center'>
              <select 
                style={{ width: 154, height: 84 }} 
                className='cursor-pointer font-semibold text-xl rounded-md bg-[#fcfcfc] shadow-md p-4' 
                defaultValue={mode} 
                value={mode} 
                onChange={(e) => {
                  setMode(e.target.value); 
                  getViews(shortedUrl.url_id, e.target.value) 
                }} 
                name="" 
                id="">
                ...
                <div className='box-p-pp'>
                  <div className='flex bg-[#fcfcfc] rounded-md p-8 shadow-lg'>
                    ...
                    <div className='pl-4'>
                      <h2 
                        style={{ fontSize: 34 }} 
                        className="text-[#404727] font-semibold font-noto">{general.views}
                      </h2>
                      <h2 
                        style={{ fontSize: 18 }} 
                        className="text-[#404727] font-regular font-noto">{TEXTS_SCHEMA[mode].views}
                      </h2>
                    </div>
                  </div>
                </div>
                <div className='box-p-pp'>
                  <div className='flex bg-[#fcfcfc] rounded-md p-8 shadow-lg'>
                    ...
                    <div className='pl-4'>
                      <h2 
                        style={{ fontSize: 34 }} 
                        className="text-[#404727] font-semibold font-noto">{general.reviews.toFixed()}
                        <span 
                          style={{ fontSize: 24 }} 
                          className="text-[#404727] font-semibold font-noto"> times
                        </span>
                      </h2>
                      <h2 
                        style={{ fontSize: 18 }} 
                        className="text-[#404727] font-regular font-noto">{TEXTS_SCHEMA[mode].reviews}
                      </h2>
                    </div>
                  </div>
                </div>
            ...
            <div className='flex-block'>
              <div className='justify-around flex p-8'>
                <div className='pt-4 chart-line bg-[#fcfcfc] rounded-md p-8 shadow-lg'>
                  <div className='p-4'>
                    <h2 
                      style={{ maxWidth: 340, fontSize: 18 }} 
                      className="text-[#404727] font-medium font-noto">{TEXTS_SCHEMA[mode].views}
                    </h2>
                  </div>
                  <Line
                    height="400px"
              ...
              <div className='pt-4 p-8 flex-block'>
                <div className='pr-4 pt-4 p-8'>
                  <div style={{ width: 340, height: 440 }} className='pt-4 bg-[#fcfcfc] rounded-md p-8 shadow-lg'>
                    <div className='p-4'>
                      <h2 
                        style={{ maxWidth: 284, fontSize: 18 }} 
                        className="text-[#404727] font-medium font-noto">{TEXTS_SCHEMA[mode].country}
                      </h2>
                    </div>
                    {
                ...
                <div className='pt-4 p-8'>
                  <div style={{ width: 340, height: 440 }} className='pt-4 bg-[#fcfcfc] rounded-md p-8 shadow-lg'>
                    <div className='p-4'>
                      <h2 
                        style={{ maxWidth: 284, fontSize: 18 }} 
                        className="text-[#404727] font-medium font-noto">{TEXTS_SCHEMA[mode].platform}
                      </h2>
                    </div>
                    {
                      platformChart.labels.length > 0 ? (
                        <Pie 
                          style={{ display: 'block', height: -140, width: 280 }} 
                          data={{
